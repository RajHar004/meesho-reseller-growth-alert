...
for month in MONTHS:
    ...
    for _ in range(ORDERS_PER_MONTH):
        ...
        quantity = rng.choices([1, 2, 3, 4], weights=[0.55, 0.25, 0.13, 0.07], k=1)[0]
...
with open(os.path.join(OUTDIR, "resellers.csv"), "w", newline="") as f:
    w = csv.DictWriter(
        f,
        fieldnames=["reseller_id", "reseller_name", "city", "region", "join_date"],
    )
    w.writeheader()
...
cur = conn.cursor()
cur.execute(
    """CREATE TABLE resellers (reseller_id TEXT PRIMARY KEY, reseller_name TEXT,
 city TEXT, region TEXT, join_date TEXT)"""
)
cur.execute(
    """CREATE TABLE orders (order_id TEXT PRIMARY KEY, reseller_id TEXT, category TEXT,
 product_name TEXT, quantity INTEGER, unit_price REAL, order_date TEXT,
...
(:order_id,:reseller_id,:category,:product_name,
 :quantity,:unit_price,:order_date,:month,:status)""",
    orders,
)
conn.commit()
conn.close()
print(
    f"Wrote {len(resellers)} resellers and {len(orders)} orders. Zero-order reseller: {ZERO_ORDER_RESELLER}"
)
